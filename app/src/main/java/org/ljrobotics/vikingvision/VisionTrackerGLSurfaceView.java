...
public class VisionTrackerGLSurfaceView extends BetterCameraGLSurfaceView implements BetterCameraGLSurfaceView.CameraTextureListener {
    ...
    public boolean onCameraTexture(int texIn, int texOut, int width, int height, long image_timestamp) {
        ...
        Log.i(LOGTAG, hRange.first + " " + hRange.second);

        int hLow = hRange.first > hRange.second ? hRange.second : hRange.first;
        int hHigh = hRange.second > hRange.first ? hRange.second : hRange.first;
        int sLow = sRange.first > sRange.second ? sRange.second : sRange.first;
        int sHigh = sRange.second > sRange.first ? sRange.second : sRange.first;
        int vLow = vRange.first > vRange.second ? vRange.second : vRange.first;
        int vHigh = vRange.second > vRange.first ? vRange.second : vRange.first;

        NativePart.processFrame(texIn, texOut, width, height, procMode, hLow, hHigh,
                sLow, sHigh, vLow, vHigh, targetsInfo);
        ...
        if (numTargets == 2) {
            NativePart.TargetsInfo.Target targetOne = targetsInfo.targets[0];
            NativePart.TargetsInfo.Target targetTwo = targetsInfo.targets[1];

            double offset = kWidth/2 - (targetOne.centroidX + targetTwo.centroidX)/2;
            ...
            if (mFpsText != null) {
                Runnable fpsUpdater = new Runnable() {
                    public void run() {
                        mFpsText.setText(targetPosition);
                    }
                };
                new Handler(Looper.getMainLooper()).post(fpsUpdater);
            }
