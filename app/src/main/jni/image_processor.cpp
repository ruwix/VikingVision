...
std::vector<TargetInfo> processImpl(int w, int h, int texOut, DisplayMode mode,
  ...
  for (auto &contour : contours) {
    ...
    if (cv::isContourConvex(convex_contour)) {
      ...
      const double kMaxTargetWidth = 80;
      const double kMinTargetHeight = 20;
      const double kMaxTargetHeight = 160;
      ...
      const double kMaxWideness = 0.6;
      const double kMinWideness = 0.2;
  ...
  const double kMaxHeightDifference = 20;
  bool found = false;
  for (int i = 0; !found && i < accepted_targets.size(); i++) {
    for (int j = 0; !found && j < accepted_targets.size(); j++) {
      ...
      double offset = abs(targetI.centroid_y - targetJ.centroid_y);
      LOGD("Offset found was %.2lf", offset);
      if (offset < kMaxOffset) {
        ...
        if (abs(rightTarget.height - leftTarget.height) < kMaxHeightDifference) {
  ...
  if (mode == DISP_MODE_RAW) {
    ...
    if (mode == DISP_MODE_TARGETS_PLUS) {
      ...
      for (auto &target : accepted_targets) {
        cv::polylines(vis, target.points, true, cv::Scalar(0, 255, 255), 3);
      }
    }
    // Render the targets
...
static void ensureJniRegistered(JNIEnv *env) {
  ...
  jclass targetsInfoClass =
      env->FindClass("org/ljrobotics/vikingvision/NativePart$TargetsInfo");
  sNumTargetsField = env->GetFieldID(targetsInfoClass, "numTargets", "I");
  sTargetsField = env->GetFieldID(
      targetsInfoClass, "targets",
      "[Lorg/ljrobotics/vikingvision/NativePart$TargetsInfo$Target;");
  jclass targetClass =
      env->FindClass("org/ljrobotics/vikingvision/NativePart$TargetsInfo$Target");
